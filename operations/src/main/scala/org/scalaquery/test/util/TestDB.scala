...
import scala.slick.jdbc.{JdbcBackend, PositionedResult, ResultSetInvoker, StaticQuery => Q}
...
abstract class TestDB(val confName: String) {
  ...
  def cleanUp() {}
  implicit def prToStringTuple3(pr: PositionedResult): (String, String, String) = (pr.nextString(), pr.nextString(), pr.nextString())
  ...
  def getLocalTables(implicit session : JdbcBackend.Session): List[String] = {
    val tables = ResultSetInvoker[(String,String,String)](_.conn.getMetaData.getTables("", "", null, null))
    tables.list.map(_._3).sorted
  }
  def assertTablesExist(tables: String*)(implicit session : JdbcBackend.Session) {
    for(t <- tables) {
  ...
  def assertNotTablesExist(tables: String*)(implicit session:JdbcBackend.Session) {
    for(t <- tables) {
      try {
  ...
  def assertUnquotedTablesExist(tables: String*)(implicit session:JdbcBackend.Session) {
    for(t <- tables) {
  ...
  def assertNotUnquotedTablesExist(tables: String*)(implicit session:JdbcBackend.Session) {
    for(t <- tables) {
      try {
...
class SQLiteTestDB(dburl: String, confName: String) extends TestDB(confName) {
  ...
  override def getLocalTables(implicit session: JdbcBackend.Session) =
    super.getLocalTables.filter(s => !s.toLowerCase.contains("sqlite_"))
}

class ExternalTestDB(confName: String, val driver: MySQLDriver) extends TestDB(confName) {
  ...
  override def cleanUpBefore() {
    if(drop.length > 0 || create.length > 0) {
      ...
      Database.forURL(adminDBURL, driver = jdbcDriver, user = configuredUserName, password = password) withSession { implicit s: JdbcBackend.Session =>
        Q.u + drop execute;
        Q.u + create execute
      }
    }
...
class AccessDB(confName: String, val driver: MySQLDriver) extends TestDB(confName) {
  val jdbcDriver = TestDBOptions.get(confName, "driver").orNull
...
abstract class DerbyDB(confName: String) extends TestDB(confName) {
  ...
  override def getLocalTables(implicit session: JdbcBackend.Session): List[String] = {
...
object TestDB {
  ...
  def H2Mem(to: DBTestObject, name: String = "test1") = new TestDB("h2mem") {
    val url = s"jdbc:h2:mem:$name;DB_CLOSE_DELAY=-1"
    val jdbcDriver = "org.h2.Driver"
    val driver = H2Driver
    override val dbName = name
  }
  ...
  def DerbyMem(to: DBTestObject) = new DerbyDB("derbymem") {
    ...
    override def cleanUp() = {
      ...
      try { Database.forURL(dropUrl, driver = jdbcDriver) withSession { s => s.conn } }
...
//  }
  ...
  def SQLServer(to: DBTestObject) = new ExternalTestDB("sqlserver", MySQLDriver) {
