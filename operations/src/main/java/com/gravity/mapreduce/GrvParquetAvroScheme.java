 ...
 */
public class GrvParquetAvroScheme<T> extends Scheme<Configuration, RecordReader, OutputCollector, Object[], Object[]> {
    ...
    public GrvParquetAvroScheme() {
        this(new Config<T>());
    }

    public GrvParquetAvroScheme(FilterPredicate filterPredicate) {
        this(new Config<T>().withFilterPredicate(filterPredicate));
    }

    public GrvParquetAvroScheme(Config<T> config) {
        this.config = config;
    }

    public GrvParquetAvroScheme(String avroSchema) {
        this(new Config<T>());
    ...
    public void sink(FlowProcess<? extends Configuration> fp, SinkCall<Object[], OutputCollector> sc)
        ...
        Object[] objectArray = GrvCascadingToAvro.parseTupleEntry(tuple, schema);
    ...
    public void sinkConfInit(FlowProcess<? extends Configuration> flowProcess, Tap<Configuration, RecordReader, OutputCollector> tap, Configuration conf) {
        ...
        jobConf.set("parquet.avro.schema", schemaStr);
