 ...
 * counters group is the {@link GrvAvroMultipleOutputs} class name. The names of the
...
public class GrvAvroMultipleOutputs{
     ...
     */
    private static final String COUNTERS_GROUP = GrvAvroMultipleOutputs.class.getName();
...
//    }
     ...
     *
     * The counters group is the {@link GrvAvroMultipleOutputs} class name.
     ...
     */
    public GrvAvroMultipleOutputs(
            TaskInputOutputContext<?, ?, ?, ?> context) {
        this.context = context;
        namedOutputs = Collections.unmodifiableSet(
                new HashSet<String>(GrvAvroMultipleOutputs.getNamedOutputsList(context)));
...
//     * otherwise returns -1.
//     * *
//     * Removed this logic with respect to Syncable.
//     * Always returns a -1.
//     * *
...
//    }
    ...
    private synchronized RecordWriter getRecordWriter(
        ...
        if (writer == null) {
            ...
            // we have a special class (GrvAvroMultipleOutputs) because of the fact that in order to use our current version
            // of libraries and multiple paths, we must set this particular setting. We cannot use the above commented out line, nor can
            // we use the avro.mo.config.namedOutput setting, although once we upgrade, chances are we'll have to
            // modify this to use the avro.mo.config.namedOutput instead
            // see https://issues.apache.org/jira/browse/AVRO-1215
    ...
    private TaskAttemptContext getContext(String nameOutput) throws IOException {
        ...
        Job job = new Job(context.getConfiguration());
        job.setOutputFormatClass(job.getConfiguration().getClass(MO_PREFIX + nameOutput + FORMAT, null, OutputFormat.class));
        ...
        // this is a terrible hack i'm adding in order to work around a problem where the key schemas are not being
        // respected
        AvroParquetOutputFormat.setSchema(job, keySchema);
        // end hack
