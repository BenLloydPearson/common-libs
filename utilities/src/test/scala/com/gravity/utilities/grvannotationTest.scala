 ...
 */
class grvannotationTest extends BaseScalaTest {

  test("Basics of Annotated") {
    import grvannotation._
    import BasicNoteTypes._

    // Here are two samples of how to return an Annotated[String]
    val sample1 = Annotated(
      "some value of any type",
      Seq(
        MsgNote("associated notes about that value"),
        new SimpleMessageMetaNote(
          "Can stash strongly typed meta data too",
          meta = (42, "Anything I want", Map.empty),
          canContinue = true)
      )
    )
    ...
    // TypeClass based, so can accept many parameter types - String, FailureResult, RecoFailure - easy to extend.
    val sample2 = "some value using string type class".annotate("This method may be less intrusive with existing code")
  ...
  test("What's lacking from ValidationNel") {
    ...
    def attempt1: ValidationNel[FailureResult, Int] = FailureResult("Live process failed").failureNel[Int]
    def attempt2: ValidationNel[FailureResult, Int] = FailureResult("Backup process failed").failureNel[Int]
  ...
  test("sequence of steps"){
    ...
    def sqrt(d: Double): ValidationNel[FailureResult, Double] = math.sqrt(d) match {
      case nan if nan == Double.NaN => FailureResult(s"Unable to calculate sqrt for $nan").failureNel
    ...
    def invert(d: Double): ValidationNel[FailureResult, Double] = {
      d match {
        case nan if nan.equals(Double.NaN) => {
          FailureResult("Can't invert NaN").failureNel
        }
        case dub => 1/dub match {
          case inf if inf.equals(Double.PositiveInfinity) => {
            FailureResult("Invert produced infinity").failureNel
          }
          case inv => {
            inv.successNel
          }
  ...
  test("Annotated with Stage"){
    import grvannotation._
    import com.gravity.utilities.grvstage._
    import BasicNoteTypes._
    ...
    case object SqrtStage extends StageWithMeta {   //reference to SqMeta breaks unless case object extends. Can't do val sqrtStage = new StageWithMeta
      ...
      def makeNote(message: String): MsgNote[Nothing] with SqrtStage.SqStageNote = new MsgNote(message) with StageNote
      ...
      def makeMetaNote(message: String, meta: MetaType, continuable: Boolean = true): SimpleMessageMetaNote[SqrtStage.SquareRootMeta] with SqrtStage.SqStageNote = new SimpleMessageMetaNote(message, meta, continuable) with StageNote
                                        // Provide an implementation to wrap your meta in a note.

      val noteCT: ClassTag[SqrtStage.SqStageNote] = classTag[StageNote]  // An unfortunately exposed implementation detail. Copy and paste this line into each Stage implementation.
    ...
    case object InversionStage extends StageWithMeta {
      ...
      def makeNote(message: String): MsgNote[Nothing] with InversionStage.InversionStageNote = new MsgNote(message) with StageNote
      def makeMetaNote(message: String, meta: MetaType, continuable: Boolean = true): SimpleMessageMetaNote[InversionStage.InversionStageMeta] with InversionStage.InversionStageNote = new SimpleMessageMetaNote(message, meta, continuable) with StageNote
      val noteCT: ClassTag[InversionStage.InversionStageNote] = classTag[StageNote]
    ...
    def sqrt(d: Double): Annotated[ValidationNel[FailureResult, Double]] = {
      ...
      //Import your stage for convenient implicits and access to makeNote and makeMetaNote as well as that stage's Note types and aliases
      //Import brings in
      // def makeNote
      // def makeMetaNote
      ...
      val result: grvannotation.Annotated[Validation[NonEmptyList[FailureResult], Double]] = math.sqrt(d) match {
        case nan if nan.equals(Double.NaN) => {
          ...
          // This is analogous to scalaz's .failureNel
        }
    ...
    //Can render meta data using custom formats defined as a partial function
    //This is much more useful for your objects than for primitives like this example
    println("Fancy custom rendering")
    computePipeline.printPF(compute64.notes){
      case d: Double => s"result was Double($d)"
    }

    //Can also use render to apply the partial function and return a RenderedStage,
    //suitable for outputting to other formats like HTML
    val renderedStages = computePipeline.render(compute64.notes) {
      case d: Double => s"result was Double($d)"
    }
  }
}
