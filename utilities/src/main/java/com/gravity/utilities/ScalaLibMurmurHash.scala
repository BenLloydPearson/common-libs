...
/* copied from scala 2.10.4 source code, because they deprecated this and we need to keep using it */
...
class ScalaLibMurmurHash[@specialized(Int,Long,Float,Double) T](seed: Int) extends (T => Unit) {
  import ScalaLibMurmurHash._
...
object ScalaLibMurmurHash {
   ...
   */
  def symmetricHash[T](xs: scala.collection.TraversableOnce[T], seed: Int): Int = {
    var a,b,n = 0
    var c = 1
    xs.seq.foreach(i => {
      val h = i.##
