...
import com.gravity.utilities.network._
import com.gravity.utilities.{GravityRoleProperties, Settings}
...
trait GridConfigurable {
	...
	def withPropertyOverrides(props: GravityRoleProperties): Repr = {
		val p = new Properties()
		props.getPropertyNames.asScala.foreach(name => Option(props.getProperty(name)).foreach(value => p.setProperty(name, value)))
		withPropertyOverrides(p)
	}
	...
	def withPropertyOverrides(props: Properties): Repr = apply(node => {
		...
		val propMap = {
			// filter+map all "grvcloud.foo.scope1.scope2" props to "foo.scope1.scope2"
			val gridProps = props.entrySet().asScala.map(e => e.getKey.toString -> e.getValue.toString).filter(_._1.startsWith("grvcloud.")).map(kv => kv._1.stripPrefix("grvcloud.") -> kv._2)
...
object GridConfigurable {
 import com.gravity.logging.Logging._
	...
	object Keys {
		val remoteExecutingFlag = "grvgrid.remote.execution"
		val gridId = "grvgrid.id"
	}
