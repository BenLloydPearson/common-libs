 ...
 */
class GridExecutor protected (gridConfig: GridConfig) extends AbstractExecutorService {
	...
	protected[grvgrid] def withNode[T](work: ViNode => T): T = {
	...
	protected[grvgrid] def shutdownUsing[T](thunk: => T): T = {
		try {
			runHooks("shutdown", _.shutdownHooks)
		} catch {
			case NonFatal(ex) => warn(ex, "Exception occurred while shutting down cloud")
		}
	...
	protected[grvgrid] def runHooks(name: String, f: GridConfigurable => Seq[Hook]) = {
		try {
