...
import com.gravity.utilities.ClosureCleaner
...
object GridTask {
	...
	private[grvgrid] def emptyCallback[T]: Try[T] => Unit = _ => {}
	...
	protected[grvgrid] def remoteCallable[T](callable: Callable[T]): Callable[Externalizer[T]] = {
		...
		new Callable[Externalizer[T]] with Serializable {
			override def call(): Externalizer[T] = {
				...
				try {
					...
					case ex: Exception =>
						warn(ex, "Exception occurred on remote call:\n")
						throw ex
				}
				result
			}
	...
	protected[grvgrid] def createClosure[T](work: => T): () => T = {
		val closure = () => work
