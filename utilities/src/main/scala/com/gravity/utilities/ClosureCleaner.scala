 ...
 *                       /___/
 *
 * Copy/paste org.apache.spark.util.ClosureCleaner  
 */
object ClosureCleaner {
 import com.gravity.logging.Logging._

	// Get an ASM class reader for a given class from the JAR that loaded it
	private[utilities] def getClassReader(cls: Class[_]): ClassReader = {
		// Copy data over, before delegating to ClassReader - else we can run out of open file handles.
		val className = cls.getName.replaceFirst("^.*\\.", "") + ".class"
		val resourceStream = cls.getResourceAsStream(className)
		// Fixme - continuing with earlier behavior ... this will always throw a IOException
	...
	private def clean(
		...
		if (!isClosure(func.getClass)) {
			warn("Expected a closure; got " + func.getClass.getName)
			return
		}

		// Improvement: clean all inner closures first. This requires us to find the inner objects.
		// Improvement: cache outerClasses / innerClasses / accessedFields

		if (func == null) {
			return
		}

		trace(s"+++ Cleaning closure $func (${func.getClass.getName}) +++")
		...
		trace(" + declared fields: " + declaredFields.size)
		declaredFields.foreach { f => trace("     " + f) }
		trace(" + declared methods: " + declaredMethods.size)
		declaredMethods.foreach { m => trace("     " + m) }
		trace(" + inner classes: " + innerClasses.size)
		innerClasses.foreach { c => trace("     " + c.getName) }
		trace(" + outer classes: " + outerClasses.size)
		outerClasses.foreach { c => trace("     " + c.getName) }
		trace(" + outer objects: " + outerObjects.size)
		outerObjects.foreach { o => trace("     " + o) }
		...
		if (accessedFields.isEmpty) {
			trace(s" + populating accessed fields because this is the starting closure")
		...
		trace(s" + fields accessed by starting closure: " + accessedFields.size)
		accessedFields.foreach { f => trace("     " + f) }
		...
		if (outerPairs.size > 0 && !isClosure(outerPairs.head._1)) {
			...
			trace(s" + outermost object is not a closure, so do not clone it: ${outerPairs.head}")
			parent = outerPairs.head._2 // e.g. SparkContext
			outerPairs = outerPairs.tail
		} else if (outerPairs.size > 0) {
			trace(s" + outermost object is a closure, so we just keep it: ${outerPairs.head}")
		} else {
			trace(" + there are no enclosing objects!")
		}

		// Clone the closure objects themselves, nulling out any fields that are not
		// used in the closure we're working on or any of its inner closures.
		for ((cls, obj) <- outerPairs) {
			trace(s" + cloning the object $obj of class ${cls.getName}")
			...
			if (cleanTransitively && isClosure(clone.getClass)) {
				trace(s" + cleaning cloned closure $clone recursively (${cls.getName})")
		...
		if (parent != null) {
			...
			if (accessedFields.contains(func.getClass) &&
				...
				trace(s" + the starting closure doesn't actually need $parent, so we null it out")
				field.set(func, null)
			} else {
		...
		trace(s" +++ closure $func (${func.getClass.getName}) is now cleaned +++")
	...
	private def ensureSerializable(func: AnyRef) {
		try {
			val oos = new ObjectOutputStream(new NullOutputStream)
			oos.writeObject(func)
			oos.close()
		} catch {
			case ex: Exception => throw new RuntimeException("Closure is not serializable", ex)
		}
	}
...
class ReturnStatementInClosureException
	extends RuntimeException("Return statements aren't allowed in closures")
 ...
 */
private[utilities] class FieldAccessFinder(
...
private class InnerClosureFinder(output: Set[Class[_]]) extends ClassVisitor(ASM5) {
	...
	// Improvement: Recursively find inner closures that we indirectly reference, e.g.
	//   val closure1 = () = { () => 1 }
